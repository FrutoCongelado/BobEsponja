...
const aguarde = new Discord.MessageEmbed()
  .setTitle('❌ Muita calma nessa hora, amigo!')
  .setColor('#F500FF')
  ...
  .setDescription(`**Por favor ${message.author}, espere **\`${time}\`** para executar outro comando**`)
    ...
    msg.delete({ timeout: 100 });
       ...
       return;
        ...
        if (!args[0]) return message.reply("Escreva algo para o Bob Esponja queimar.")
                        img.then(image => {
                          jimp.loadFont(jimp.FONT_SANS_32_BLACK).then(font => {
                       image.resize(700, 800)
                                           image.print(font, 80, 200, args.join(" "), 518)
                image.getBuffer(jimp.MIME_PNG, (err, i) => {
                    message.channel.send({files: [{ attachment: i, name: "bobfogueira.png"}]})
                })
            })
        })
    }